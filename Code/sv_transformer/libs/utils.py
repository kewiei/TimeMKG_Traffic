...
def predict_and_save_results(net, data_loader, data_target_tensor, epoch, _max, _min, params_path, type):
    ...
    with torch.no_grad():
        ...
        for batch_index, batch_data in enumerate(data_loader):
            ...
            # encode
            encoder_output = net.encode(encoder_inputs)
            input.append(encoder_inputs[:, :, :, 0:2].cpu().numpy())  # (batch, T', 1)
        ...
        _max = np.transpose(_max, (0,1,3,2)) #(B,N,T,F)
        _min = np.transpose(_min, (0,1,3,2)) #(B,N,T,F)
...
def load_graphdata_normY_channel1(graph_signal_matrix_filename, num_of_hours, num_of_days, num_of_weeks, DEVICE, batch_size, shuffle=True, percent=1.0):
    ...
    train_decoder_input_tensor = torch.from_numpy(train_decoder_input).type(torch.FloatTensor).to(DEVICE)  # (B, N, F, T)
    ...
    #  ------- val_loader -------
    val_decoder_input_start = val_x[:, :, 0:2, -1:]  # (B, N, 1(F), 1(T)),最后已知traffic flow作为decoder 的初始输入
    ...
    val_decoder_input_tensor = torch.from_numpy(val_decoder_input).type(torch.FloatTensor).to(DEVICE)  # (B, N, F, T)
    val_target_tensor = torch.from_numpy(val_target_norm).type(torch.FloatTensor).to(DEVICE)  # (B, N, F, T)
    ...
    test_decoder_input_tensor = torch.from_numpy(test_decoder_input).type(torch.FloatTensor).to(DEVICE)  # (B, N, F, T)
    test_target_tensor = torch.from_numpy(test_target_norm).type(torch.FloatTensor).to(DEVICE)  # (B, N, F, T)
...
def load_graphdata_normY_connector(graph_signal_matrix_filename, num_of_hours, num_of_days, num_of_weeks, DEVICE, batch_size, shuffle=True, percent=1.0):
    '''
    将x,y都处理成归一化到[-1,1]之前的数据;
    每个样本同时包含所有监测点的数据，所以本函数构造的数据输入时空序列预测模型；
    该函数会把hour, day, week的时间串起来；
    ...
    file_data = graph_signal_matrix_filename
    # print(file_data)
    test_x = file_data['test']['x'] # (10181, 307, 3, 12)
    #test_x = test_x[:, :, 0:1, :]
    test_target = file_data['test']['target'] # (10181, 307, 12)
    test_timestamp = file_data['test']['timestamp'] # (10181, 1)
    test_x_length = test_x.shape[0]
    scale = int(test_x_length*percent)
    print('ori length:', test_x_length, ', percent:', percent, ', scale:', scale)

    _max = file_data['stats']['_max']  # (1, 1, 3, 1)
    _min = file_data['stats']['_min']  # (1, 1, 3, 1)
    ...
    return test_loader, test_target_tensor, _max, _min
